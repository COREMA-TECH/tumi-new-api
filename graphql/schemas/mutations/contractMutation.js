...
const generateContractTerms = (contract, businessCompany, supervisor, posRate) => {
   ...
   let { Start_Week, End_Week, Contract_Expiration_Date, Contract_Start_Date} = businessCompany;
...
const generateExhibit = (Exhibit, contract, businessCompany, supervisor) => {
   let currentTime = moment();
   let { Start_Week, End_Week, Contract_Expiration_Date, Contract_Start_Date} = businessCompany;
   let starDay = moment().isoWeekday(Start_Week || 0).format('dddd'), endDay = moment().isoWeekday(End_Week || 0).format('dddd');
...
const ContractMutation = {
    addContract: {
        ...
        resolve(source, args) {
           console.log(args); // TODO: (LF) Quitar console log
           let contracts = args.contracts.map(c => {
              console.log(c.Id_Entity); // TODO: (LF) Quitar console log
              let businessCompanyId = c.Id_Entity || c.IdManagement;
              Db.models.BusinessCompany.findOne({
                 where: {
                    Id: businessCompanyId
                  },
                  include: [{
               ...
               //c.Contract_Terms = generateContractTerms(c);
               return c;
            })

            return null;

            // return Db.models.Contracts.bulkCreate(contracts, { returning: true }).then((output) => {

...
async function InsBusinessCompanies (args) {
  try {
	  	if (args)
    		{
    			Strquery = 'INSERT INTO public."BusinessCompany" ("Code", "Code01","Id_Contract","Id_Company","BusinessType","Name","Description","Start_Week","End_Week","Start_Date","Legal_Name","Country","State","City","Id_Parent","IsActive","User_Created","User_Updated","Date_Created","Date_Updated","ImageURL","Location","Location01","Rate","Zipcode", "Fax", "Phone_Prefix", "Phone_Number", "Primary_Email","Contract_URL", "Insurace_URL", "Other_URL", "Other01_URL","Other_Name","Other01_Name","Suite","Rooms","Contract_Status") VALUES('+ args.input.Code +','+ args.input.Code01 +',' + args.input.Id_Contract +',' + args.input.Id_Company +','+args.input.BusinessType+','+args.input.Name +','+args.input.Description +','+args.input.Start_Week+','+args.input.End_Week+','+args.input.Start_Date+','+args.input.Legal_Name+','+args.input.Country+','+args.input.State+','+args.input.City+','+args.input.Id_Parent+','+args.input.IsActive+','+args.input.User_Created+','+args.input.User_Updated+','+args.input.Date_Created+','+args.input.Date_Updated+','+args.input.ImageURL+','+args.input.Location+','+args.input.Location01+','+args.input.Rate+','+args.input.Zipcode+','+args.input.Fax+','+args.input.Phone_Prefix+','+args.input.Phone_Number+','+args.input.Primary_Email +','+args.input.Contract_URL +','+args.input.Insurace_URL+','+args.input.Other_URL +','+args.input.Other01_URL +','+args.input.Other_Name +','+args.input.Other01_Name +','+ args.input.Suite +','+ args.input.Rooms + ','+ args.input.Contract_Status +') RETURNING "Id", "Code", "Code01","Id_Contract","Id_Company","BusinessType","Name","Description","Start_Week","End_Week","Start_Date","Legal_Name","Country","State","City","Id_Parent","IsActive","User_Created","User_Updated","Date_Created","Date_Updated","ImageURL","Location","Location01","Rate","Zipcode", "Fax", "Phone_Prefix", "Phone_Number", "Primary_Email","Contract_URL", "Insurace_URL", "Other_URL", "Other01_URL","Suite","Contract_Status"'
...
async function getContacts (args) {
  try {
		...
		if (args.Id_Entity>=0) {strparam3= args.Id_Entity  ;}
		     ...
		     Strquery = 'select * from public.vwContacts Where  "IsActive" = coalesce('+ strparam1 +',"IsActive") and  "Id_Entity" = coalesce('+ strparam3 +',"Id_Entity")  and "Id" = coalesce('+ strparam2 +',"Id")'
    		console.log(Strquery);
...
async function InsContacts (args) {
  try {
    		...
    		{
    	console.log(args);
    			Strquery = 'INSERT INTO public."Contacts" ("Id_Entity", "First_Name", "Middle_Name", "Last_Name", "Electronic_Address", "Phone_Number", "Contact_Type" , "Contact_Title", "IsActive", "User_Created", "User_Updated", "Date_Created", "Date_Updated", "Id_Supervisor", "Id_Deparment") VALUES('+ args.input.Id_Entity +','+ args.input.First_Name +','+ args.input.Middle_Name +','+ args.input.Last_Name  +',' +args.input.Electronic_Address +','+args.input.Phone_Number+','+args.input.Contact_Type +','+ args.input.Contact_Title +','+args.input.IsActive +','+args.input.User_Created+','+args.input.User_Updated+','+args.input.Date_Created+','+args.input.Date_Updated+','+args.input.Id_Supervisor+','+args.input.Id_Deparment+') RETURNING "Id","Id_Entity", "First_Name","Middle_Name","Last_Name", "Electronic_Address", "Phone_Number", "Contact_Type","Contact_Title" , "IsActive", "User_Created", "User_Updated", "Date_Created", "Date_Updated", "Id_Supervisor", "Id_Deparment"'
    		}
    	    else{console.log("Error Insert Data");}
